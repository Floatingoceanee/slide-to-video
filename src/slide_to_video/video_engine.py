...
class VideoEngine(object):
    ...
    def burn_subtitles_to_video(
        ...
        Uses subprocess + raw FFmpeg instead of ffmpeg-python because the
        ``subtitles`` filter's force_style option does not pass through
        ffmpeg-python correctly (comma/colon escaping issues).
        ...
        # FFmpeg filter syntax uses ':' and ',' as separators.
        # Escape ':' in path for Windows drive letters (e.g. C\:).
        # Wrap force_style value in single quotes to protect commas.
        safe_srt_path = os.path.relpath(srt_path).replace('\\', '/').replace(':', '\\:')
        vf_filter = f"subtitles={safe_srt_path}:force_style='{force_style}'"
